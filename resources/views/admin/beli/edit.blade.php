...
<div class="content-wrapper">
    ...
    <section class="content-header">
      ...
      <ol class="breadcrumb">
        <li><a href="{{ url('beli/create') }}" class="active"><i class="fa fa-dashboard"></i> Tambah Data Pembelian</a></li>
      </ol>
    </section>
        ...
        <div class="col-md-12">
          ...
          <div class="box box-primary">
            <div class="box-header with-border">
              <h3 class="box-title">Form Input Beli</h3>
            </div>
            <!-- /.box-header -->
            <!-- form start -->
            @if(session('status'))
            ...
            @endforeach
             <form class="form-horizontal" method="POST" action="{{ url('beli') }}">
                {{ csrf_field() }}


                <div class="form-group{{ $errors->has('tglBeli') ? ' has-error' : '' }}">
                    <label for="tglBeli" class="col-md-4 control-label">Tanggal Beli</label>

                    <div class="col-md-6">
                     <input id="tglBeli" type="date" class="form-control" name="tglBeli" value="{{ old('tglBeli') }}" required autofocus>

                        @if ($errors->has('tglBeli'))
                            <span class="help-block">
                                <strong>{{ $errors->first('tglBeli') }}</strong>
                            </span>
                        @endif
                    </div>
                </div>
